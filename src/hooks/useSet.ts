...
export function useSet<T>(initialValue: T[] | Set<T> = new Set()): [Set<T>, SetActions<T>] {
  ...
  const actions = useMemo<SetActions<T>>(() => ({
    add: (item) => setSet((prev) => {
      if (prev.has(item)) return prev;
      const next = new Set(prev);
      next.add(item);
      return next;
    }),
    remove: (item) => setSet((prev) => {
      if (!prev.has(item)) return prev;
      const next = new Set(prev);
      next.delete(item);
      return next;
    }),
    toggle: (item) => setSet((prev) => {
      const next = new Set(prev);
    ...
    removeMultiple: (items) => setSet((prev) => {
      const next = new Set(prev);
      let changed = false;
      for (const item of items) {
        if (next.has(item)) {
